...
export function FeaturedProjectBlockComponent({ data, theme }: BlockProps<FeaturedProjectBlockData>) {
  const d = data
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row items-center gap-10 lg:gap-16 mt-16">
          <AnimatedContent direction="horizontal" distance={-60} delay={200} className="lg:w-3/5">
            <div ref={visualRef} className="aspect-[4/3] rounded-2xl bg-gradient-to-br from-warm-500/30 via-accent-pink-500/20 to-violet-500/30 flex items-center justify-center relative overflow-hidden">
              <div className="text-center relative z-10">
                <div className="text-8xl mb-6">{d.project.icon || '🚀'}</div>
                <p className={`text-3xl font-bold font-display ${isDark ? 'text-white/60' : 'text-stone-700/60'}`}>
                  {d.project.title}
                </p>
              </div>
            </div>
          ...
          <AnimatedContent direction="horizontal" distance={60} delay={400} className="lg:w-2/5">
            ...
            <h3 className={`text-2xl sm:text-3xl font-bold mt-3 mb-4 ${theme.textColor}`}>{d.project.title}</h3>
            <p className={`leading-relaxed mb-6 ${isDark ? 'text-stone-400' : 'text-stone-500'}`}>{d.project.desc}</p>
