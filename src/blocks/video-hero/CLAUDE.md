...
10k-websites 方法论落地：滚动驱动视频正放/倒放（scroll-scrub）+ 分段字幕带 + 可读性系统 + 五档静态降级。
...
- **可读性**（用户要求字幕无局部背景，砍掉了 per-caption scrim）：全局径向 scrim（静态 CSS）→ 三层 text-shadow → kicker chip 胶囊。CSS 在组件内 `<style>`（scoped class vh-*）。
