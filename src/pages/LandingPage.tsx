...
export default function LandingPage() {
  ...
  useMagnetic(ctaPrimaryRef, { strength: 0.3, radius: 100 })
  useMagnetic(ctaSecondaryRef, { strength: 0.15, radius: 90 })

  // feature 卡片视差（错落层次）——按固定顺序调用 hook
  const f0 = useRef<HTMLDivElement>(null)
  const f1 = useRef<HTMLDivElement>(null)
  const f2 = useRef<HTMLDivElement>(null)
  useParallax(f0, { speed: 0.1 })
  useParallax(f1, { speed: 0.2 })
  useParallax(f2, { speed: 0.15 })

  // 定价「推荐」卡 3D 浮起 + 视差
  const featuredPricingRef = useRef<HTMLDivElement>(null)
  useParallax(featuredPricingRef, { speed: 0.25 })

  // 联系区二维码呼吸浮动
  const qrcodeRef = useRef<HTMLDivElement>(null)
  useIdleFloat(qrcodeRef, { amplitude: 6, duration: 4 })

  return (
    <div className="relative min-h-screen overflow-hidden bg-[#fdf6ec]">
      <CursorGlow />
      {/* 氛围背景层：浅色 Aurora + 柔光晕，提升质感 */}
      <div className="pointer-events-none fixed inset-0 z-0 opacity-70">
        <Aurora
          colorStops={['#fde68a', '#fbcfe8', '#ddd6fe']}
          amplitude={0.6}
          speed={0.3}
          blend={0.6}
        />
      </div>
      {/* 顶部柔光渐隐，让 Aurora 与内容过渡更自然 */}
      <div className="pointer-events-none fixed inset-0 z-0 bg-gradient-to-b from-[#fdf6ec]/40 via-transparent to-[#fdf6ec]/60" />
      ...
      <div className="relative z-10">
        ...
        <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
          <div className="relative z-10 text-center px-6 max-w-4xl mx-auto">
            <AnimatedContent direction="vertical" distance={20} delay={100}>
              <div className="inline-block px-4 py-1.5 rounded-full bg-white/60 backdrop-blur-md text-warm-700 text-sm font-medium mb-8 shadow-sm shadow-amber-300/30 ring-1 ring-white/60">
                ✨ 高动效 × 轻量化 × 专属定制
              </div>
            </AnimatedContent>

            {/* 标题：GSAP SplitText 分行入场（替代 BlurText），节奏更克制高级 */}
            <SplitText
              text="你的专属高定网页"
              className="font-display text-4xl sm:text-6xl lg:text-7xl font-bold tracking-tight text-stone-900 mb-6 leading-tight"
              splitType="lines"
              from={{ y: 60, opacity: 0 }}
              to={{ y: 0, opacity: 1 }}
              delay={150}
            ...
            <AnimatedContent direction="vertical" distance={40} delay={900}>
              <div className="flex flex-col sm:flex-row justify-center gap-4">
                {/* 磁吸主 CTA：移除 CSS hover translate，由 GSAP 接管交互反馈，避免与磁吸 x/y 抢 transform */}
                <a
                  ref={ctaPrimaryRef}
                  href="#demos"
                  data-magnetic
                  className="group relative px-8 py-4 bg-gradient-to-r from-warm-500 to-accent-pink-500 text-white font-bold rounded-2xl transition-shadow duration-300 text-lg cursor-pointer overflow-hidden shadow-lg shadow-amber-500/25 hover:shadow-xl hover:shadow-amber-500/40"
                >
                  <span className="relative z-10">查看 Demo</span>
                  {/* hover 扫光效果（子元素 transform，与磁吸外层不冲突） */}
                ...
                <a
                  ...
                  href="#contact"
                  data-magnetic
                  className="glass-card inline-block px-8 py-4 text-stone-700 font-bold hover:bg-white/90 transition-colors duration-300 text-lg cursor-pointer"
                >
                  立即咨询
                </a>
              </div>
          ...
          <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10">
            <div className="w-6 h-10 rounded-full border-2 border-stone-400/50 flex items-start justify-center p-1.5">
              <div className="w-1 h-2 rounded-full bg-stone-400/70 animate-bounce" />
            </div>
          </div>
        </section>
        ...
        <section id="features" className="py-24 px-6">
          <div className="max-w-6xl mx-auto">
            <SplitText
              text="为什么选择 VBSite"
              className="font-display text-3xl sm:text-4xl lg:text-5xl font-bold text-center mb-4 gradient-text bg-gradient-to-r from-warm-500 to-accent-pink-500 bg-clip-text text-transparent"
              splitType="lines"
              delay={60}
              duration={1}
            />
            ...
            {/* 用 stagger 统一编排三张卡片入场 + 视差层次 */}
            <AnimatedContent direction="vertical" distance={50} stagger={0.15}>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {[
                  ...
                  <div key={item.title} ref={[f0, f1, f2][i]}>
                    <GlassCard className="p-8 text-center transition-shadow duration-300 hover:shadow-xl hover:shadow-amber-200/40">
                      <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${item.bg} flex items-center justify-center text-3xl text-white mx-auto mb-6 shadow-lg`}>
                        {item.icon}
                      </div>
        ...
        <section id="demos" className="py-24 px-6">
          <div className="max-w-6xl mx-auto">
            ...
            <AnimatedContent direction="horizontal" distance={-60} stagger={0.2}>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <a href="/resume" className="demo-preview block group rounded-2xl overflow-hidden hover:scale-[1.03] hover:shadow-2xl hover:shadow-amber-500/20 transition-all duration-500 cursor-pointer ring-1 ring-white/40">
                  <div className="aspect-[4/3] bg-gradient-to-br from-warm-100 via-warm-200 to-accent-pink-300/20 relative">
                    ...
                    <div className="absolute top-4 right-4 px-3 py-1 rounded-full bg-warm-500 text-white text-sm font-medium shadow-md">暖色系</div>
                  </div>
                ...
                <a href="/portfolio" className="demo-preview block group rounded-2xl overflow-hidden hover:scale-[1.03] hover:shadow-2xl hover:shadow-violet-500/20 transition-all duration-500 cursor-pointer ring-1 ring-white/40">
                  <div className="aspect-[4/3] bg-gradient-to-br from-stone-900 via-stone-800 to-violet-900/30 relative">
                    ...
                    <div className="absolute top-4 right-4 px-3 py-1 rounded-full bg-violet-500 text-white text-sm font-medium shadow-md">深色系</div>
                  </div>
                  <div className="p-6 bg-white/60 backdrop-blur">
        ...
        <section id="pricing" className="py-24 px-6">
          <div className="max-w-4xl mx-auto">
            ...
            <AnimatedContent direction="vertical" distance={60} stagger={0.2}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {[
                  ...
                  <div
                    key={plan.label}
                    ref={plan.featured ? featuredPricingRef : undefined}
                    className={`p-8 sm:p-10 rounded-2xl transition-shadow duration-300 hover:shadow-xl ${plan.featured ? 'bg-gradient-to-br from-warm-500/10 to-accent-pink-500/10 border border-warm-500/40 shadow-lg shadow-amber-500/10' : 'bg-white/60 backdrop-blur border border-white/40'}`}
                  >
                    {plan.featured && <div className="inline-block px-4 py-1 bg-gradient-to-r from-warm-500 to-accent-pink-500 text-white text-xs font-bold rounded-full mb-4 shadow-md">推荐</div>}
        ...
        <section id="contact" className="py-24 px-6">
          <div className="max-w-3xl mx-auto text-center">
            <SplitText
              text="准备好了吗？"
              className="font-display text-3xl sm:text-4xl lg:text-5xl font-bold mb-6 text-stone-900"
              splitType="lines"
              delay={60}
              duration={1}
            />
            ...
            <AnimatedContent direction="vertical" distance={40} scale={0.95}>
              <GlassCard className="p-8 sm:p-12 max-w-md mx-auto">
                <div ref={qrcodeRef} className="w-48 h-48 mx-auto rounded-2xl bg-gradient-to-br from-warm-100 to-accent-pink-400/10 flex items-center justify-center mb-6">
