...
- `SingleUserSpacePage.tsx` — 单用户构建模式页面（`VITE_BUILD_USER`）。接收已注入的 config 直接渲染（admin 预览的渲染蓝本）。
- `StarPage.tsx` — GSAP 高动效 Demo 页（`/star`）。
- `admin/` — 管理后台（`/admin`：登录/客户列表/新建/可视化编辑器）。整体 lazy 加载。详见 [admin/CLAUDE.md](admin/CLAUDE.md)。
